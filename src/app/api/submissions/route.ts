  ...
  try {
    ...
    if (team.attemptsUsed >= 3) {
      return NextResponse.json(
        { error: "Maximum attempts reached (3 of 3 attempts used). Further submissions are locked." },
        { status: 400 }
      );
    }

    const body = await req.json();
    const { codeContent, filename, approachNotes } = body;

    if (!codeContent || !codeContent.trim()) {
      return NextResponse.json({ error: "Code content cannot be empty." }, { status: 400 });
    }

    const currentAttempt = team.attemptsUsed + 1;
    ...
    const submission = await db.submission.create({
      data: {
        ...
        filename: filename || `attempt_${currentAttempt}.py`,
        codeContent,
    ...
    await db.auditLog.create({
      data: {
        ...
        details: `Team ${team.teamName} completed Attempt ${currentAttempt}/3. Auto-Score: ${evalResult.autoScore}. Similarity: ${maxSimilarity}%.`,
      },
    });
